...
try:
  tag = client.create_tag(package_name="hello-world", version=version, tag="stage")
except:
  print("stage tag is already on this version")

tag = client.create_tag(package_name="hello-world", version=version, tag="stage-"+datetime.datetime.now(datetime.UTC).strftime("%Y%m%dT%H%M%SZ"))
