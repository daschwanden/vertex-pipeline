...
if firstVersion:
  ...
  if not source_tag:
    ...
    tag = client.create_tag(package, version, os.environ.get("BRANCH_NAME").replace("/", "-")+"-"+datetime.datetime.now(datetime.UTC).strftime("%Y%m%dT%H%M%SZ")+"-"+os.environ.get("SHORT_SHA", "latest"))
